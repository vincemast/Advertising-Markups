...
Regression_output_N <- function(Data,naics,N){
  ...
  #make time and coefficient table? might be better to just store model 9 coefficients (and clusted SEs) and make plots outside.
...
################# clean industry names, store names  #################
  ...
  #adding the space to end changes NA to string, replace so regression drops NA values
  temp_data$industry<-na_if(temp_data$industry,"NA ")
  

  
  #get number of observations for each industry
  Ind_count_temp<-temp_data %>% count(industry)
...
################# run regressions 1_9, store ugly table  #################
  ...
  ## Model 7 - industry, industry intercept, time trend, time quad (neither interacted)
  model_7 <- feols(
    Adr_MC ~ i(industry,MU_1) + time +time2 -MU_1| industry, 
    data = temp_data
  )
  
  ## Model 8 - industry, industry intercept, time trend, time quad (both interacted)
  model_8 <- feols(
    Adr_MC ~ i(industry,MU_1) + MU_1*time + MU_1*time2 + time + time2 - MU_1| industry, 
    data = temp_data
  )
  
  ## Model 9 - industry, industry intercept, time dummy (+ interacted)
  model_9 <- feols(
    Adr_MC ~ i(industry,MU_1) + i(fyear,MU_1,ref=2022) - MU_1| industry +fyear, 
    data = temp_data
  )
...
################# make nice table, store  #################
  ...
  #coefficient labels
  labels_temp= c("Intercept","MU-1",indnames_table,"time","time × (MU-1)","time²","time² × (MU-1)") 
  #defining names is how coef_maps identify which variable name youre referring to
  names(labels_temp)=c(
    names(model_2$coefficients[1:2]),                       #intercept&MU
    names(model_9$coefficients[1:length(indnames_temp)]), #industry
    names(model_8$coefficients[(length(model_8$coefficients)-3):length(model_8$coefficients)]) #time interacted
  )
  
  
  #define summary stats i want to keep
  sumstatdrop_temp=("AIC|BIC|RMSE|R2 Within|R2 Within Adj.")
  
  # define columns want to keep; ie the first 2 (MU and intercept) + number of industries, + 4 (time, time^2 and interact)
  ...
  new_row_temp <- data.frame("Year × (MU-1) (2022 reference)","","","","","","X")
  
  #make table
  table_temp<-modelsummary(models_temp, coef_omit=-keepcol_temp, coef_map=labels_temp, gof_map = gm_temp, add_rows = new_row_temp,
                        fmt = 4,stars=TRUE)
  
  #make table (latex)
  table_temp_latex<-modelsummary(models_temp, coef_omit=-keepcol_temp, coef_map=labels_temp, gof_map = gm_temp, add_rows = new_row_temp,
                           fmt = 4,stars=TRUE,
                           output="latex")
  ...
  #Nice table, ugly table, model 9 sector coefficients (well named), model 9 year coefficients (well named), models, latex table,
  #obs per industry
  
  
  #grab year coefficents
  year_cos<-data.frame(model_9$coefficients[(numberOfCategories_temp+1):length(model_9$coefficients)])
  year_cos$year <- as.numeric(
    gsub('fyear::','',
    gsub(':MU_1','',rownames(year_cos)))
         )
  rownames(year_cos) <- NULL
  names(year_cos)<- c('fit','year')
  year_cos$se <-  summary(model_9, cluster = "industry")$se[(numberOfCategories_temp+1):length(model_9$coefficients)]
  ...
  sector_cos$se <-  summary(model_9, cluster = "industry")$se[1:numberOfCategories_temp]
  #sector_cos$se <-  summary(model_9, cluster = "GVKEY")$se[1:numberOfCategories_temp]
  
  #make into list
  output<-list(table_temp,ugly_table,sector_cos,year_cos,models_temp,table_temp_latex,Ind_count_temp)
...
Sector_time_coefs_N <- function(Data,naics,N){
  ...
  for (i in industries_temp$industry[!is.na(industries_temp$industry)]){
    ...
    model_temp <- feols(
      ...
      cluster="GVKEY",
      data = subset(temp_data, industry== i)
    )
    
    #save, need to clean name of year
    ceos_temp<-data.frame(gsub(":MU_1",'',
                          gsub("fyear::",'',names(model_temp$coefficients))
                          ))
