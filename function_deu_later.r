...
second_stage_exp <-
  function(panel, yvar, xvars, xvar_l,
    ...
    result <- optim(theta_init, momment_exp, #nolint
                    ...
                    input = input,
                    control = list(
                      trace = FALSE, maxit = 1e9, reltol = 1e-20
                                   #,factr = 1e8 #nolint
                                   )) #nolint
...
acf_rolling_window_exp <-
  function(temp, yvar, xvars, fszvars, zvars, orthogx, r, block = 20) { #nolint
    ...
    for (i in 1:length(sectors)) { #nolint
      ...
      setup <- set_up(yvar, xvars,  fszvars, zvars, orthogx, pdata_window)
      pdata_window <- setup$data
      xvar_l <- setup$xvar_l
      fs_rhs <- setup$fs_rhs
